    ...
    for ( j in 1:5 ) {
        ...
        for ( i in 2:22 ){                                            // For loop designed to run simulations to fill Et, xt, LP, and dLP matrices
          ...
          if ( xt[i-1, j] <= 0 ) {                                    // If statement telling the program to set xt, LP, and dLP to 0 for the rest of the simulation if they ever reach or exceed 0
              ...
              LP[i-1, j] = (LPmax * ((xt[i-1, j]/0.5)^1) * ((1 - xt[i-1, j])/(1 - 0.5))^1);                                    // LP
              dLP[i-1, j] = (LP[i-1, j] * (LP[i-1, j] - Et[i-1, j]) * ((1/xt[i-1, j]) - (1/(1 - xt[i-1, j]))));                // dxdt2
              xt[i,j] = xt[i-1, j] + (1) * (LP[i-1, j] - Et[i-1, j]) + ((1)^2) * (0.5) * dLP[i-1, j];                          // xt+1
              if (xt[i-1,j] >= 0.999){
