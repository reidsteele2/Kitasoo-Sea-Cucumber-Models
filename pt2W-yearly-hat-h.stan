data{
    ...
    matrix[21, 5] HY;                 // matrix of harvest in each site (column) for each year, duplicated to put into months (row)
    ...
    matrix[21, 5] mweights;           // Mean biosample weights for each site and year
    matrix[21, 5] sdweights;          // SD biosample weights for each site and year
    row_vector[5] V;                  // Virgin individuals estimate for each site
}

parameters{
    real<lower=0.01,upper=0.25> LPmax;            // Maximum latent productivity
    real<lower=0.002,upper=5> G;                  // Geometric mean of biomass density (kg/m^2)
    real<lower=0.011,upper=0.754> Wsyt[21,5];     // Sea cucumber average weight, bounded by max and min
    real<lower=0.01,upper=2.3> sigS;              // Standard deviation of random site effects
    real<lower=0.01,upper=2.3> sigY;              // Standard deviation of random year effects
    real<lower=0.01,upper=2.3> sigT;              // Standard deviation of random transect effects
    real hat_Tst[87];                                 // Random transect effects
    real hat_Yy[21];                                  // Random year effects
    real hat_Ss[5];                                   // Random site effects
    real T;                                       // Tst hierarchy
    real Y;                                       // Yy hierarchy
    real S;                                       // Ss hierarchy
    real<lower=0.001> nn;                         // Shaping parameter, from Quinn and Deriso (1999)
}

transformed parameters{
    real Tst[87];                                 // Random transect effects
    real Yy[21];                                  // Random year effects
    real Ss[5];                                   // Random site effects
    matrix[22, 5] xt;                                            // Matrix of x(t) values
    matrix[21, 5] LP;                                            // Matrix of LP values
    matrix[21, 5] dLP;                                           // Matrix of dLP values
    matrix[21, 5] Et;                                            // Matrix of harvest rate
    real<lower=0.001> n;                                         // Shaping parameter, set up with nn to make it impossible for it to be 1 (undefined)
    if ( nn == 1 ) {                                             // n is the shaping parameter used. If nn is 1, n = nn + 0.000001 to avoid undefined issues, otherwise they are equal
      n = nn + 0.000001;
    }
    else {
      n = nn;
    ...
    for ( j in 1:5 ) {
        ...
        for ( i in 2:22 ){                                            // For loop designed to run simulations to fill Et, xt, LP, and dLP matrices
          ...
          LP[i-1, j] = (LPmax * (n^(n/(n-1)))/(n-1) * xt[i-1, j] - LPmax * (n^(n/(n-1)))/(n-1) * (xt[i-1, j])^n);                                      // Calculation of LP
          dLP[i-1, j] = ( ( LP[i-1, j] - Et[i-1, j] ) * (-((LPmax * (n^(n/(n-1))))*(n*(xt[i-1, j]^n) - xt[i-1, j]))/((n-1) * xt[i-1, j])));            // Calculation of dx/dt
          xt[i, j] = xt[i-1, j] + (1) * (LP[i-1, j] - Et[i-1, j]) + ((1)^2) * (0.5) * dLP[i-1, j];                                                     // Calculation of xt+1
              if (xt[i-1,j] >= 0.999){
...
model{
    ...
    sigT ~ uniform( 0.01 , 2.3 );                                 // sigT prior, noninformative, from Hajas (2011)
    sigY ~ uniform( 0.01 , 2.3 );                                 // sigY prior, noninformative, from Hajas (2011)
    sigS ~ uniform( 0.01 , 2.3 );                                 // sigS prior, noninformative, from Hajas (2011)
