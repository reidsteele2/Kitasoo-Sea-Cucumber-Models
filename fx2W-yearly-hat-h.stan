...
transformed parameters{
    ...
    for ( j in 1:5 ) {
        ...
        for ( i in 2:22 ){                                            // For loop designed to run simulations to fill Et, xt, LP, and dLP matrices
          ...
          LP[i-1,j] = -exp(1) * LPmax * xt[i-1,j] * log(xt[i-1,j]);                                                    // Calculation of LP
          dLP[i-1,j] = (LP[i-1,j] - Et[i-1,j]) * (-exp(1) * LPmax * ( log ( xt[i-1,j] ) + 1 ));                        // Calculation of dx/dt
          xt[i, j] = xt[i-1,j] + (1) * (LP[i-1,j] - Et[i-1,j]) + ((1)^2) * (0.5) * dLP[i-1,j];                         // Calculation of x(t+1)
              if (xt[i-1,j] >= 0.999){
